...
import pandas as pd
import numpy as np
from utils import print_gpu_memory, check_environment, process_data
...
'''
# Description
Inference with Google MedGemma 4B from Hugging Face
...
def parse_arguments():
    parser = argparse.ArgumentParser(description="Run inference with MedGemma 4B on smmile dataset")
    parser.add_argument(
        "--dataset",
        choices=["augmented"],
...
if not HF_TOKEN:
    print("Warning: HF_TOKEN environment variable not set. This may cause issues with private datasets.")
...
def run_inference(icl_questions_with_images, output_dir, args):
    ...
    for i, chunk in enumerate(tqdm(icl_questions_with_images, desc="Generating Responses")):
        ...
        # Add system message for medical expertise
        messages.append({
            "role": "system",
            "content": [{"type": "text",
                         "text": "You are an expert medical professional with extensive knowledge in medical imaging, diagnosis, and clinical practice."}]
        })
        ...
        try:
            ...
            if task_format == "mcqa":
                ...
                # For open format, just store the response
                generated_answers.append(response.strip())

            full_responses.append(response if isinstance(response, str) else "")
    ...
    if task_format == "mcqa":
        ...
        output_filename = os.path.join(output_dir, f'result_medgemma_4b_{inference_mode}_mcqa.csv')

        # Print MCQA accuracy
        correct_count = sum(is_correct[:min_length])
        total_count = min_length
        accuracy = correct_count / total_count if total_count > 0 else 0
        print(f"\nMCQA Results Summary:")
        ...
        output_filename = os.path.join(output_dir, f'result_medgemma_4b_{inference_mode}_open.csv')
...
if __name__ == "__main__":
    ...
    print(f"Running inference with MedGemma 4B in {args.inference_mode} mode")
