...
# Set environment variables for cache directories BEFORE importing transformers
# os.environ['HF_HOME'] = os.path.expanduser('~/.cache/huggingface')
# os.environ['TRANSFORMERS_CACHE'] = os.path.expanduser('~/.cache/huggingface')
# os.environ['SENTENCE_TRANSFORMERS_HOME'] = os.path.expanduser('~/.cache/huggingface')
...
def run_inference(icl_questions_with_images, args, output_dir):
    ...
    print(f"Loading model: {output_name}...")
    try:
        ...
        print(f"\nError loading model {output_name}: {e}")
    ...
    for i, chunk in tqdm(enumerate(icl_questions_with_images), total=len(icl_questions_with_images)):
        ...
        # Construct messages based on inference mode
        if inference_mode == "0-shot":
            ...
            # Prepare inputs - for ICL, we need to use all images
            try:
                input_text = processor.apply_chat_template(messages, add_generation_prompt=True)
        ...
        try:
            ...
            # Extract just the generated part (model's answer)
            generated_part = processor.decode(output[0][len(inputs['input_ids'][0]):], skip_special_tokens=True)

            # For MCQA, process and evaluate the answer
            if task_format == "mcqa":
                # For the first example only, print some debug info
                if i == 0:
                    print("\nDEBUG - Template format sample:")
                    print(input_text[:300] + "..." if len(input_text) > 300 else input_text)
                    print("\nDEBUG - Generated response sample:")
                    print(generated_part[:300] + "..." if len(generated_part) > 300 else generated_part)

                # First try standard extraction method from generated_part (preserving original behavior)
                # This ensures compatibility with previously working cases
                standard_extraction = extract_mcqa_answer(generated_part, current_problem_id, None)

                # Only if standard extraction fails, try the new text matching approach
                if standard_extraction != "EXTRACTION_FAILED":
                    selected_option = standard_extraction
                else:
                    # Try text matching with options on generated part
                    selected_option = extract_mcqa_answer(generated_part, current_problem_id, options)

                    # If still failed, try using the full response
                    if selected_option == "EXTRACTION_FAILED":
                        print(f"Trying to extract from full response for problem {current_problem_id}")
                ...
                generated_answers.append(generated_part)
    ...
    if task_format == "mcqa":
        ...
        output_filename = os.path.join(output_dir, f'result_{output_name}_{inference_mode}_mcqa.csv')

        # Write accuracy to a summary file
        summary_file = os.path.join(output_dir, f'accuracy_summary_{output_name}_{inference_mode}_mcqa.txt')
        ...
        output_filename = os.path.join(output_dir, f'result_{output_name}_{inference_mode}_open.csv')
...
if __name__ == "__main__":
    ...
    os.environ['SENTENCE_TRANSFORMERS_HOME'] = args.hf_cache_dir
