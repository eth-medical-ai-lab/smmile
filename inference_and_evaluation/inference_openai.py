...
# # Helper: turn a local file into a data URL
...
# Per‐model configuration - ANONYMIZED
MODEL_SETTINGS = {
    "gpt-4o": {
...
async def main():
    parser = argparse.ArgumentParser(
        description="Send an image + prompt to an Azure OpenAI vision‐enabled model"
    )
    parser.add_argument(
        "--model",
        required=True,
        choices=MODEL_SETTINGS.keys(),
        help="One of: " + ", ".join(MODEL_SETTINGS.keys()),
    )
    parser.add_argument(
        "--inference_mode",
        type=str,
    ...
    cfg = MODEL_SETTINGS[args.model]

    if not cfg["api_key"]:
        raise ValueError("Azure OpenAI API key not found in environment variables. Please set AZURE_OPENAI_API_KEY.")
    if not cfg["azure_endpoint"]:
        raise ValueError("Azure OpenAI endpoint not found in environment variables. Please set AZURE_OPENAI_ENDPOINT.")

    client = AsyncAzureOpenAI(api_key=cfg["api_key"], api_version=cfg["api_version"],
    ...
    print(f"\nStarting inference in {args.inference_mode} mode with {args.task_format} format...")
    ...
    for i, chunk in tqdm(enumerate(icl_questions_with_images), total=len(icl_questions_with_images)):
        ...
        if args.task_format == "mcqa":
            ...
            formatted_target_question = format_mcqa_question(chunk[-1]['question'], options)
        else:
            # For open format, use the original question
            formatted_target_question = chunk[-1]['question']
            options = None
            correct_option = None

        if args.inference_mode == "0-shot":
            # Convert PIL image to base64
            data_url = get_data_url(chunk[-1]['image'])

            messages = [
                {"role": "system", "content": "You are a highly skilled physician and an excellent writer."},
                {
                    "role": "user",
                    "content": [
                        {"type": "text",
                         "text": formatted_target_question if args.task_format == "mcqa" else chunk[-1]['question']},
                        {"type": "image_url", "image_url": {"url": data_url, "detail": "high"}},
                    ],
                }
            ]
        else:
            messages = [{"role": "system", "content": "You are a highly skilled physician and an excellent writer."}]
            for idx, example in enumerate(chunk[:-1]):
                data_url = get_data_url(example['image'])
        ...
        if cfg["supports_params"]:
            # these params apply to GPT‐4 variants and GPT‐4.1
            call_kwargs.update({
                "temperature": 0.0,
                "max_tokens": 512,
                "logprobs": True,
        ...
        try:
            ...
            if args.task_format == "mcqa":
                ...
                selected_option = extract_mcqa_answer(response_content, current_problem_id, options)
                ...
                generated_answers.append(response_content)
    ...
    if args.task_format == "mcqa":
        ...
        output_filename = os.path.join(output_dir, f'result_{args.model}_{args.inference_mode}_mcqa.csv')
    else:
        ...
        output_filename = os.path.join(output_dir, f'result_{args.model}_{args.inference_mode}_open.csv')
